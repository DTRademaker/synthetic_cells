...
# Authors: Daniel Rademaker & Joshua Koopmans
...
def round_red_blood_cell(*args):
    ...
    seeds = [int(random.random() * 10e5), int(random.random() * 10e5)]
    center = (50, 50)
    outer_radius = 16
    middle_radius = 14
    inner_radius = 13
...
def generate_images(func, nmb_cells, out_filename: str):
    """
    ...
    imgs = p.map(func, range(nmb_cells))
...
if __name__ == "__main__":
	# Uncomment next line to draw samples of round blood cell images:
	# generate_sample_images(round_red_blood_cell)

	# Uncomment next line to draw samples of bullet blood cell images:
	# generate_sample_images(bullet_red_blood_cell)

	# Generate small dataset of round and bullet cells:
	generate_images(func=round_red_blood_cell, nmb_cells=64, out_filename=prefix + "round_cells")
	generate_images(func=bullet_red_blood_cell, nmb_cells=64, out_filename=prefix + "bullet_cells")
